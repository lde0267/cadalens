...
기본 입력: data/cadastral/derived/anseong_37714092_parcels_within.gpkg
           data/satellite/georef/(B060)정사영상_2025_37714092_georef.tif

예시:
    python scripts/clip_parcel_chips.py                 # 임야 큰 것 10개
    python scripts/clip_parcel_chips.py --jimok 대 --limit 20
    python scripts/clip_parcel_chips.py --jimok all --select random --limit 10
    python scripts/clip_parcel_chips.py --no-square --pad-m 3
"""
...
# --- PROJ/GDAL 데이터 경로 격리 (다른 스크립트와 동일) ------------------------
...
import cv2  # 배열 리샘플 전용 (파일 I/O 에는 쓰지 않음: 한글 경로 문제)
import numpy as np
...
DEF_OUT = Path("data/chips/37714092")
