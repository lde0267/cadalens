...
# -*- coding: utf-8 -*-
"""
C2 프로토타입 — MaskCLIP식 dense per-patch + 내부 max/fraction 풀링.

A1(내부 토큰만 CLS 요약)의 한계: 큰 필지가 절반만 전환돼도 CLS 평균이 "숲"으로 수렴.
C2: CLS를 안 쓰고 패치 하나하나를 텍스트공간으로 뽑아(patch_vec) 각각 built/forest
    점수를 매긴 뒤, 폴리곤 내부에서 **fraction / max** 로 집계. "필지의 몇 %가
    건물처럼 보이나" 가 나오므로 부분 전환이 평균에 안 묻힌다.

파이프라인(= A1 + C2):
  1) _embeds 후 폴리곤 커버리지>=keep-tau 패치 + CLS 만 남김 (A1)
...
입력 : data/chips/37714092/*.png (mask, RGBA alpha=폴리곤).  프롬프트/텍스트 임베딩은
       classify_imya_binary 의 POSITIVE/NEGATIVE 그대로 -> v1/A1 과 직접 비교.

사용:
    python scripts/classify_imya_c2.py --cpu
    python scripts/classify_imya_c2.py --cpu --frac-tau 0.30 --max-tau 0.85 --last-mlp
"""
...
from imya.classify_binary import PROMPT_SETS  # noqa: E402

DEF_CHIPS = Path("data/chips/37714092")
DEF_OUT = Path("data/remoteclip/37714092_imya_c2.csv")
V1_CSV = Path("data/remoteclip/37714092_imya_binary.csv")
A1_CSV = Path("data/remoteclip/37714092_imya_binary_a1.csv")
JIMOK = "임"
