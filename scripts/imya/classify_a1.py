...
# -*- coding: utf-8 -*-
"""
A1 프로토타입 — RemoteCLIP ViT 에서 필지 폴리곤 '바깥' 패치토큰을 드롭.
...
입력  : data/chips/37714092/*.png  (mask 모드, RGBA. alpha=폴리곤 마스크)
스코어: classify_imya_binary 의 POSITIVE/NEGATIVE/템플릿/2-way softmax 그대로.
        이미지 임베딩 계산 경로만 교체 -> v1(전체 49토큰) 과 직접 비교.

사용:
    python scripts/classify_imya_a1.py --cpu
    python scripts/classify_imya_a1.py --cpu --tau 0.1 --min-keep 3
"""
...
DEF_OUT = Path("data/remoteclip/37714092_imya_binary_a1.csv")
...
def main() -> None:
    ...
    ap.add_argument("--tau", type=float, default=0.5, help="패치 유지 커버리지 임계")
    ...
    print(f"A1 | {args.arch} ({grid}x{grid}) | {device} | 임야 칩 {len(files)} | "
          f"prompts={args.prompts} ({len(POSITIVE)}/{len(NEGATIVE)}) | tau={args.tau} "
