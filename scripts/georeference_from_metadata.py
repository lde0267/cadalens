...
사용
----
    python scripts/georeference_from_metadata.py                 # data/satellite/*.tif 자동
    python scripts/georeference_from_metadata.py \
        --tif "data/satellite/(B060)정사영상_2025_37714092.tif" \
        --xml "data/satellite/(B060)정사영상메타데이터_202511001237714092.xml"
"""
...
# rasterio 를 import 하기 전에 내장 데이터 폴더로 강제 고정한다.
...
def main() -> None:
    ...
    ap.add_argument("--sat-dir", type=Path, default=Path("data/satellite"))
    ap.add_argument("--outdir", type=Path, default=Path("data/satellite/georef"))
    ap.add_argument("--margin", type=float, default=50.0,
                    help="도엽번호 방식에서 도곽 여백(m). 기본 50 (NGII 표준).")
    ap.add_argument("--cadastre", type=Path,
                    default=Path("data/cadastral/LSMD_CONT_LDREG_경기_안성시/"
                                 "LSMD_CONT_LDREG_41550_202608.shp"))
    args = ap.parse_args()
