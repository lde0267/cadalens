...
| `scripts/s3_make_chips.py` | 필지 폴리곤으로 224px 칩 생성 | `chips/<sheet>_<mode>/` (`index.csv` + `<pnu>_<jibun>.png` + `_montage.png`) |
...
`--outdir` 미지정 시 `chips/<sheet>_<mode>/` 로 자동. 파이프라인이 이 규칙으로 칩을 찾아 재사용한다.
...
| `finalize_labels.py` | `labels.json` → `labels.csv` + `임야/형질변경/보류/` 폴더 분류 + (있으면) 파이프라인 결과 채점 |

```bash
python 01_preprocess/scripts/labeling/make_label_set.py
# labels/imya_eval_150/label_tool.html 을 브라우저로 열고 1/2/3 키 → "결과 저장" → labels.json
